...
3.  [NEW] HRIS partner triggers an automatic enrollment sequence on a specific
    device using /users/cloud-enroll.
4.  User completes biometric enrollment on the physical device (Face and
    Fingerprint).
...
| -------------- | ------ | -------- | --------------------- |
| `api_token`    | string | Yes      | Partner API token     |
| `device_id`    | string | Yes      | Target ACAS device ID |
| `hris_user_id` | string | Yes      | Unique HRIS user ID   |
| `first_name`   | string | Yes      | User first name       |
| `last_name`    | string | Yes      | User last name        |
